...
public class HourlyTipsExercise {
    ...
    public JobExecutionResult execute() throws Exception {
        ...
        DataStream<Tuple3<Long, Long, Float>> sumDriverTips = fares.keyBy(fare -> fare.driverId)
                .window(TumblingEventTimeWindows.of(Time.hours(1))).process(new TaxiRideMax());
        ...
        DataStream<Tuple3<Long, Long, Float>> hourlyMax =
                sumDriverTips.windowAll(TumblingEventTimeWindows.of(Time.hours(1))).maxBy(2);
        hourlyMax.addSink(sink);
        // the results should be sent to the sink that was passed in
        // (otherwise the tests won't work)
        // you can end the pipeline with something like this:
        // DataStream<Tuple3<Long, Long, Float>> hourlyMax =
        // fares.keyBy(taxifare -> taxifare.startTime)
        // .flatMap(new RichFlatMapFunction<TaxiFare, Tuple3<Long, Long, Float>>() {
        // @Override
        // public void flatMap(TaxiFare value,
        // Collector<Tuple3<Long, Long, Float>> out) throws Exception {
        // out.collect(new Tuple3<>(value.getEventTimeMillis(), value.rideId,
        // value.tip));
        // }
        // });
        // hourlyMax.keyBy(value -> value.f0).addSink(sink);

        // execute the pipeline and return the result
        return env.execute("Hourly Tips");
    }

    public static class TaxiRideMax
            extends ProcessWindowFunction<TaxiFare, Tuple3<Long, Long, Float>, Long, TimeWindow> {

        @Override
        public void process(Long driverId, Context context, Iterable<TaxiFare> elements,
                Collector<Tuple3<Long, Long, Float>> out) {
            float sumTip = 0;
            for (TaxiFare element : elements) {
                sumTip = sumTip + element.tip;
            }
            out.collect(Tuple3.of(context.window().getEnd(), driverId, sumTip));
        }
    }
}
